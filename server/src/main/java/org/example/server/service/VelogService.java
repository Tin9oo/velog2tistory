...
import org.example.server.util.VelogParser;
...
public class VelogService {
    ...
    public ArrayList<VelogInfo.List> getVelogList(String username) {
        ...
        return VelogParser.parseList(response);
    }

    public ArrayList<VelogInfo.Post> getVelogPost(ArrayList<VelogInfo.List> velogList, String username) {
        WebClient webClient = WebClient.builder().build();

        ArrayList<VelogInfo.Post> velogPosts = new ArrayList<>();
        VelogRequest.Post post;
        String response;
        for (int i = 0; i < velogList.size(); i++) {
             post = VelogRequest.Post.builder()
                    .operationName("ReadPost")
                    .variables(
                            VelogRequest.PostVariables.builder()
                                    .username(username)
                                    .url_slug(velogList.get(i).getUrlSlug())
                                    .build()
                    )
                    .query(QUERY_POST)
                    .build();
            ...
            velogPosts.add(VelogParser.parsePost(response));
        }
